...
bright_spot_deltas <- df %>%
  ...
  mutate(
    mean_exposure          = median(exposure_scaled, na.rm = TRUE),
    mean_sensitivity       = median(sensitivity_scaled, na.rm = TRUE),
    mean_adaptive_capacity = median(adaptive_capacity_scaled, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  mutate(
  ...
  # keep ONLY countries that match bright-spot criteria: Lower Exposure, higher Sensitivity, lower Adaptive Capacity than regional median
...
ggplot(  aes(x = Country,
  ...
  geom_text(
    ...
    aes(x = Country,
        y = Percent_Change ),   # 5 units above the bar
    label = "*",
    size = 6,
    fontface = "bold",
    inherit.aes = FALSE
