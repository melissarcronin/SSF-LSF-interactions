...
###############################################################################
#LOAD LIBRARIES, SET UP WORKSPACE ########
###############################################################################

# COMPONENT 1. EXPOSURE ########
###############################################################################
### crit_1_1 Composite index of intensity of nearshore large-scale fishing activity 3####
### crit_1_1_A AIS-broadcasting vessel density ####
...
AIS_dir <- here("data","GFWFishing25km")

# SUMFishing25kmBuff.csv
csv_path <- file.path(AIS_dir, "SUMFishing25kmBuff.csv")
...
#create Criteria 1.1.A (AIS-broadcasting vessel density)
...
### crit 1_1_B Ratio of non-broadcasting fishing to AIS-broadcasting fishing ####
#Ratio of LSF fishing activity that is not broadcasting AIS (AIS activity is missing from public monitoring systems) to fishing activity that is publicly tracked with 25 km of shore

SAR_dir <- here("data","SAR_matched_data")
...
#histogram 
crit_1_1_B_histogram<-crit_1_1_B_data %>% 
ggplot()+
...
### crit_1_2 Contribution to global marine SSF production (scaled to nearshore area , IHH) #####
...
#visualize relationship between exposure and its component variables
ggplot(exposure_data_df) +
...
###############################################################################
### crit_2_1 Composite index of employment and economic dependence on the small-scale fisheries sector ####

### crit_2_1_A Contribution to SSF marine fisheries landed value (% of global marine SSF landed value, IHH) ######
...
### crit_2_1_B Contribution to SSF global marine fisheries employment (% of global marine SSF employment, IHH) ######
...
### crit_2_1_C Contribution of marine SSF employment in fisheries to total employment (% of country’s labor force, IHH)####
...
### crit_2_1_D Contribution to marine subsistence employment (work for own consumption) (% of global subsistence workers, IHH) #####
...
# plot
crit_2_1_histogram<-ggplot(crit_2_1_data) +
...
### crit_2_2 Composite index of coastal nutritional dependence on SSFs ######
### crit_2_2_A Contribution to marine SSF production within country (kg/  coastal cap /yr, IHH) [for coastal population within 100 km]####

coastal_population<- read.csv("coastal_pop_2010.csv", sep="," ,header=T)
...
### crit_2_2_C Prevalence of inadequate micronutrient intake (PMII) for 14 micronutrients  ######
#Prevalence of inadequate micronutrient intake  (Beale et al 2017) 

crit_2_2_C_data<-read.csv("prevalence_micronutrient_deficiency.csv", sep=",", header=T) %>% 
...
# plot
crit_2_2_C_histogram<-ggplot(crit_2_2_C_data) +
...
#use adaptive_capacity_inverse in ultimate calculations
...
###############################################################################
# MERGE 3 COMPONENTS INTO ONE DF  ########
###############################################################################
...
#calculate simplified SIFI index using sum, prod, and mean of 3 components. 
#These scores will be compared against the more rigorous index methods (PCA and CFA)
